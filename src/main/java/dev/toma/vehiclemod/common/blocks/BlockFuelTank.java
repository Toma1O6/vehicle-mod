...
import dev.toma.vehiclemod.util.JsonGenerator;
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (stack.isEmpty()) {
            ...
            if (FuelHandler.instance().isFuel(stack)) {
                ...
                if (!player.isSneaking() && te.fuelLevel < TileEntityFuelTank.MAX_FUEL_LEVEL) {
            ...
            if (stack.getItemDamage() > 0 && te.fuelLevel >= 20) {
                stack.setItemDamage(stack.getItemDamage() - 1);
                te.onFuelUpdate(world, state, -20);
            }
        }
        return false;
    }
